...
export default async (container) =>{
  ...
  const set = await container.setsRepository.create(sneakerConfig.sets[0],{brand:brand})
  ...
  const href3 = await container.hrefsRepository.create()
   ...
   return await container.sneakersRepository.create({sneaker_id:sneakerConfig.sneaker_id, model: sneakerConfig.model, addition: sneakerConfig.addition},
     {description: description, brands: [brand], sets: [set], extended: extended, image:image,prices: [price1,price2,price3], hrefs: [href1,href2,href3]})
}
